	...
	private drawTilemap(): void {
		for (let col = 0; col <= 64; col++) {
			for (let row = 0; row <= 64; row++) {
				let sprite: number = this.sceneTilemapComponent.tiles[row * this.scene.columns + col];

				this.context.drawImage(this.scene.tilesets[0].image, sprite * this.scene.tileSize, 0, this.scene.tileSize, this.scene.tileSize, col * this.scene.tileSize, row * this.scene.tileSize, this.scene.tileSize, this.scene.tileSize);
			}
		}
	}
				...
				if (spriteRendererComponent) {
					...
					if (spriteRendererComponent.row !== undefined) {
						this.context.drawImage(
							this.scene.tilesets[0].image, // TODO: Remove this hardcoded index.
